...
def main():
    ...
    resp = ebs_client.start_snapshot(VolumeSize=volume_gib, Description="NixOS direct upload")
    ...
    ami_name = f"nixos-{base_name}-{int(time.time())}"

    print(f"Registering AMI: {ami_name}...")
    ami_response = ec2_client.register_image(
        Name=ami_name,
        Description=f"NixOS image created from {src}",
        Architecture=arch,
